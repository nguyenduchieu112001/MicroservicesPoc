...
export default {
  ...
  data() {
    return {
      ...
      productsFilterOptions: [{ text: "All products", value: "ALL" }],
      filteredPeriod: {
  ...
  methods: {
    fetchTotalSales() {
      HTTP.post("dashboard/total-sales", this.setupRequest({})).then(
        (response) => {
    ...
    fetchAgentsSales() {
      HTTP.post("dashboard/agents-sales", this.setupRequest({})).then(
        (response) => {
          var amounts = {};
          var counts = {};
          Object.getOwnPropertyNames(response.data.perAgentTotal).forEach(
            (val) => {
    ...
    fetchSalesTrends() {
      HTTP.post("dashboard/sales-trends", this.setupRequest({})).then(
        (response) => {
    ...
    productFilterChange(option) {
      ...
      this.filterProductsSelectionText = _.find(
    ...
    changeDatesFilter(option) {
      ...
      if (option === "LAST_12_MONTHS") {
        ...
        this.filteredPeriod.endDate = moment();
        this.filteredPeriod.startDate = moment()
          .month(0)
          .date(1);
    ...
    setupRequest(request) {
      request.salesDateFrom = moment(this.filteredPeriod.startDate).format(
        "YYYY-MM-DD"
      );
      request.salesDateTo = moment(this.filteredPeriod.endDate).format(
        "YYYY-MM-DD"
      );
      request.unit =
        this.filteredPeriod.option === "THIS_MONTH" ? "Week" : "Month";
      request.productCode =
        this.filterProductsSelection === "ALL"
          ? null
          : this.filterProductsSelection;
      return request;
    },
  },
};
